...
void setup() 
{
  ...
  servo1.attach(GRAB_UNDER_SERVO);  // 로봇팔 회전해주는 모터
  servo2.attach(GRAB_UPPER_SERVO);  // 로봇팔
  servo3.attach(BASKET_LEFT_SERVO); // 왼쪽 바구니
  servo4.attach(BASKET_RIGHT_SERVO);  // 오른쪽 바구니

  digitalWrite(IN1, 0);
  digitalWrite(IN2, 0);
  analogWrite(EN1, 0);  // 컨베이어
  
  digitalWrite(IN3, 0);
  digitalWrite(IN4, 0);
  analogWrite(EN2, 0);  // 피니언 랙
